...
Epsilon = 2.5e-4;   % termination criterion
...
for iter = 1:Max_iter           % number of iteration
    ...
    for i = 1:M_est             % for R1 ~ RM
        ...
        cvx_end
        % computing error
        square_error = square_error + (1/N^2)*norm(Ri-R(:,:,i),'fro');
        % write back
        R(:,:,i) = Ri;
        % pring information
    ...
    % check convergence
    square_error = square_error / M_est;
...
if Skip
    ...
    cvx_begin 
        variable A_estimate(N,N)
        CROSS = 0;

        % for commutatively-enforced term
        for i = 2:M_est
            CROSS = CROSS + square_pos(norm((A_estimate*R(:,:,i) - R(:,:,i)*A_estimate),'fro'));
        end

        % minimum objective function
        minimize((1/2)*square_pos(norm(R(:,:,1)-A_estimate,2)) + lambda1*sum(sum(abs(A_estimate))) + lambda3*CROSS);
    cvx_end
...
if M_est == 1
    ...
    cvx_begin 
        variable c_estimate(((M_est+1)*(M_est+2)/2)-3,1)
        OBJ = 0;
        ...
        for k = M_est+1:K                 % K-M stop predictor
           ...
           OBJ = OBJ + sum((Y-B*c_estimate).^2);
        end

        % minimum objective function
        minimize((1/2)*OBJ + lambda2*sum(abs(c_estimate)));
    cvx_end
    c_estimate = [0;1;c_estimate];
end
